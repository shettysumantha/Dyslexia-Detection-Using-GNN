...
def extract_features_and_labels(graph, label):
    ...
    if label is not None:
        labels = [label for _ in range(num_nodes)]
...
class App(tk.Tk):
    ...
    def predict_dyslexia(self, train_features):
        ...
        dyslexia_detected = self.predict_dyslexia_from_csv(self.selected_filename, train_features)

        if dyslexia_detected == "Dyslexia":
            self.update_result_text("Symptoms of Dyslexia detected")
        elif dyslexia_detected == "Control":
            self.update_result_text("No Symptoms of Dyslexia detected")
        else:
            self.update_result_text("Error in prediction")


    def predict_dyslexia_from_csv(self, filename, train_features):
        try:
            ...
            # Recreate the model and compile it
            input_dim = train_features.shape[1]
            hidden_dim = 64
            output_dim = 2  # Binary classification
            loaded_model = GraphSAGEModel(input_dim, hidden_dim, output_dim)
            loaded_model.compile(optimizer='adam', loss='sparse_categorical_crossentropy', metrics=['accuracy'])

            # Load the model weights
            loaded_model.load_weights('graph_sage_model_weights.h5')

            G = create_graph_from_data(df)
            features, _ = extract_features_and_labels(G, None)
            features = tf.reshape(features, (features.shape[0], features.shape[1]))
...
if __name__ == "__main__":
    app = App()
    app.predict_dyslexia(train_features)
    app.mainloop()
